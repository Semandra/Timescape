 ...
 */
console.log ("Load custom painter");
Timeline.OriginalEventPainter = function(params) {
    ...
    this._eventIdToElmt = {};
	this._direction = ("direction" in params) ? params.direction : Timeline.REVERSE; // Semandra - Reverse entries parameter
};
...
Timeline.OriginalEventPainter.prototype.paint = function() {
   ...
   impreciseIconMargin: eventTheme.instant.impreciseIconMargin
	...
	// Semandra - added direction toggle
...
Timeline.OriginalEventPainter.prototype.paintPreciseInstantEvent = function(evt, metrics, theme, highlightIndex) {
    ...
    var labelRight = labelLeft + labelSize.width + theme.event.label.rightBuffer; //Semandra - right padding
    
    var rightEdge = labelRight;
    //var track = this._findFreeTrack(evt, rightEdge);
	// Semandra - toggle for direction added
	...
	this._tracks[track] = this._direction == Timeline.FORWARD ? rightEdge : iconLeftEdge; //Semandra - toggle for direction
};
...
Timeline.OriginalEventPainter.prototype.paintPreciseDurationEvent = function(evt, metrics, theme, highlightIndex) {
	...
	this._tracks[track] = this._direction == Timeline.FORWARD ? rightEdge : startPixel; //Semandra - toggle for direction
};
...
Timeline.OriginalEventPainter.prototype._paintEventLabel = function(evt, text, left, top, width,
    ...
    labelDiv.style.width = (width+5) + "px"; //Semandra - added space for IE 9 display

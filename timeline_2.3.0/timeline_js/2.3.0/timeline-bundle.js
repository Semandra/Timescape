...
Timeline.CompactEventPainter.prototype.paintStackedPreciseInstantEvents = function(T, j, c, E) {
    ...
    var F = L(T[0].getStart()); //Set left position
    var Y = [];
    ...
    var Z = k * j.trackHeight + j.trackOffset; // Set track position
...
Timeline.HotZoneGregorianEtherPainter.prototype.zoom = function(A) {
    if (A != 0) {
        for (var B = 0; B < this._zones.length;
            ++B) {
            if (this._zones[B]) {
...
Timeline.OriginalEventPainter.prototype.paint = function() {
    ...
    var
        A = (this._highlightMatcher != null) ? this._highlightMatcher : function(K) {
            return -1;
        };
    var E = C.getEventReverseIterator(D, B);
    while (E.hasNext()) {
...
Timeline.OriginalEventPainter.prototype._paintEventIcon = function(K, B, C, G, F, D) {
    ...
    //	var eventClass = " ";
    //	if (K._start == K._latestStart) { eventClass = eventClass + "singleEventStartKnown ";};
    //	if (K._end == K._earliestEnd) { eventClass = eventClass + "singleEventEndKnown "};	
...
Timeline.OriginalEventPainter.prototype._paintEventTape = function(N, B, D, A, G, C, J, I, M) {
    ...
    var OB = this._timeline.getDocument().createElement("div");

    //Set class of type of inpercise event tape-- start date Know; end date known; neither known
    //	console.log (N);
    //	console.log (J);
    //	console.log (I);
    //	console.log ("N="+N+" B="+B+" D="+D+" A="+A+" G="+G+" C="+C+" J="+J +" I="+I+" M="+M);
    //			console.log(N._start)
    //			console.log(N._latestStart)
    //			console.log(N._earliestEnd)
    //			console.log(N._end)

    var tapeClass = "tapeLayer" + M + " ";
    if (N._start == N._latestStart) {
        tapeClass = tapeClass + "startKnown ";
    };
    if (N._end == N._earliestEnd) {
    ...
    //		console.log("------------")
    OB.className = this._getElClassName("timeline-event-tape " + tapeClass, N, "tape"); //Semandra - added a identifying label to class between tape layers
    //console.log ("N="+N+" B="+B+" D="+D+" A="+A+" G="+G+" C="+C+" J="+J +" I="+I+" M="+M);


    OB.id = this._encodeEventElID("tape" + M, N);
    OB.style.left = D + "px";
    OB.style.width = F + "px";
    OB.style.height = E + "px";
    OB.style.top = K + "px";
    if (N._title != null) {
        OB.title = N._title;
    }
    if (G != null) {
        OB.style.backgroundColor = G;
    }
    var L = N.getTapeImage();
    var H = N.getTapeRepeat();
    H = H != null ? H : "repeat";
    if (L != null) {
        OB.style.backgroundImage = "url(" + L + ")";
        OB.style.backgroundRepeat = H;
    }
    SimileAjax.Graphics.setOpacity(OB, C);
    this._eventLayer.appendChild(OB);
    return {
        left: D,
        top: K,
        width: F,
...
/*Timeline.OriginalEventPainter.prototype._paintEventTape=function(N,B,D,A,G,C,J,I,M){
	...
	O.className=this._getElClassName("timeline-event-tape "+ "tapeLayer"+M,N,"tape"); //Semandra - added a identifying label to class between tape layers
	console.log (O);
...
Timeline.DefaultEventSource.Event.prototype = {
    ...
    fillWikiInfo: function(D) {
        ...
        //D.appendChild(document.createTextNode("[")); //Creates "Discuss" link in bubble - removed by Semandra
        //D.appendChild(A);
        //D.appendChild(document.createTextNode("]"));
    },


    // ------ Add Time to display bubble ----------
    fillTime: function(A, B) {

        //var date = new Date(B.labelPrecise(this._start));
        //console.log(date.format('mmm d, yyyy'));
        var dateStart = new Date(B.labelPrecise(this._start));
        var dateLatestStart = new Date(B.labelPrecise(this._latestStart));
        var dateEarlyEnd = new Date(B.labelPrecise(this._earliestEnd));
        var dateEnd = new Date(B.labelPrecise(this._end));
     ...
     //   dateEnd = dateEnd.format('mmm d, yyyy');
        ...
        //A.appendChild(A.ownerDocument.createTextNode(this._obj.displayDate));   // Semandra - created to have control over diplay date 

        // REMOVED in favour of using a user field "displayDate"	--  Semandra
       	if(this._instant){
        		if(this.isImprecise()){
        			
        			A.appendChild(A.ownerDocument.createTextNode(dateStart));
        		...
        		if(this.isImprecise()){
        			...
        			//A.appendChild(A.ownerDocument.createTextNode(dateStart+" ~ "+dateLatestStart));
        			A.appendChild(A.ownerDocument.createTextNode(" ~ " + dateStart));
        			A.appendChild(A.ownerDocument.createTextNode(" to "));
        			//A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._earliestEnd)+" ~ "+B.labelPrecise(this._end)));
        			A.appendChild(A.ownerDocument.createTextNode(" ~ "+dateEnd));
        		}else{
        			//A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._start)));
        			A.appendChild(A.ownerDocument.createTextNode(dateStart));
        			A.appendChild(A.ownerDocument.createTextNode(" to "));
...
Timeline.ClassicTheme._Impl = function() {
    ...
    this.event = {
        ...
        instant: {
            icon: Timeline.urlPrefix + "images/dp-transparent.png",
            iconWidth: 10,
            iconHeight: 10,
            impreciseOpacity: 20,
